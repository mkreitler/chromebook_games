...
jb.setPixi = function(app) {
  jb.pixiRenderer = app.renderer;

  app.ticker.add((dt) => {
    jb.loop();
  });
};
jb.clearCanvas = function(canvas, color) {
  if (canvas) {
...
jb.drawGradientRect = function(canvas, x, y, w, h, isVertical, colorStops) {
  const ctxt = canvas.getContext('2d');
...
jb.drawRoundedRect = function(canvas, x, y, w, h, r, borderColor, fillColor, borderWidth) {
  const ctxt = canvas.getContext('2d');
...
jb.pixiSpriteFromCanvas = function(canvas) {
  const base = new PIXI.BaseTexture.from(canvas),
