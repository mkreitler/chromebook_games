...
jb.setAntiAliasing = function(doAntiAlias) {
  if (jb.ctxt) {
    jb.ctxt.imageSmoothingEnabled = doAntiAlias;
    jb.ctxt.mozImageSmoothingEnabled = doAntiAlias;
  }
};
...
jb.glyphs = {
    ...
    addNew: function(key, name, pixelData, replace=false) {
      var success = false;

        if (/[0-9]+[x][0-9]+/.exec(key) && jb.glyphs[key] && (replace || typeof(jb.glyphs[key][name]) === 'undefined')) {
          var newGlyph = {image: null, defaultBounds: null, pixelData: pixelData};
          jb.glyphs[key][name] = newGlyph;
          success = true;
        }
    ...
    palettes: [
        ...
        ["rgba(0, 0, 0, 1)", "rgba(255, 255, 255, 1)", "rgba(64, 64, 64, 1)", "rgba(128, 128, 128, 1)",
         ...
         "rgba(192, 178, 128, 1)", "rgba(128, 0, 128, 1)", "rgba(128, 0, 0, 1)", "rgba(0, 128, 0, 1)",
         "rgba(0, 0, 255, 1)", "rgba(128, 128, 0, 1)", "rgba(0, 0, 128, 1)", "rgba(96, 48, 0, 1)"],
...
jb.EZglyphs = {
  ...
  generate: function(cellSize, shapeData, colorData, lineWidth, backFill, palettes) {
    ...
    this.setPalettes(palettes || this.DEFAULT_PALETTE);
